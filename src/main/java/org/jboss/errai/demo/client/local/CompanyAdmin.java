    ...
    this.companyBox.addValueChangeHandler(new ValueChangeHandler<User>(){
      ...
      public void onValueChange(ValueChangeEvent<User> event){
        ...
        if(!userWithAccess.isEmpty()){
          ...
          accessBox.setValue(userWithAccess.get(0), true);
          accessBox.setAcceptableValues(userWithAccess);
        }else{
          accessBox.setEnabled(false);
          accessBox.setValue(new User("prazdne",  Arrays.asList(new Role(UsersRole.COMPANY))));
          accessBox.setAcceptableValues(new ArrayList<User>());
        }
        ...
        if(!availableUsers.isEmpty()){
          ...
          userBox.setValue(availableUsers.get(0), true);
          userBox.setAcceptableValues(availableUsers);
        }else{
          userBox.setEnabled(false);
          userBox.setValue(new User("prazdne",  Arrays.asList(new Role(UsersRole.COMPANY))));
          userBox.setAcceptableValues(new ArrayList<User>());
        }
